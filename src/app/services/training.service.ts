...
import { City, Branch, Block, Budget, CreateCityRequest, CreateBranchRequest, CreateBlockRequest } from '../models/location.model';
import { Training, TrainingAssignmentRequest, TrainingAssignmentResult } from '../models/training.model';
...
export class TrainingService extends BaseService {
  ...
  createCity(request: CreateCityRequest): Observable<City> {
    ...
    return this.http.post<ApiResponse<City> | City>(API_ENDPOINTS.CITIES.CREATE, request).pipe(map(r => this.unwrap(r) as City));
  }
  ...
  createBranch(request: CreateBranchRequest): Observable<Branch> {
    ...
    return this.http.post<ApiResponse<Branch> | Branch>(API_ENDPOINTS.BRANCHES.CREATE, request).pipe(map(r => this.unwrap(r) as Branch));
  }
  ...
  createBlock(request: CreateBlockRequest): Observable<Block> {
    ...
    return this.http.post<ApiResponse<Block> | Block>(API_ENDPOINTS.BLOCKS.CREATE, request).pipe(map(r => this.unwrap(r) as Block));
  }
  ...
  assignTraining(request: TrainingAssignmentRequest): Observable<TrainingAssignmentResult> {
    if (environment.useMockData) {
      ...
      const city = MOCK_CITIES.find(c => c.cityId === request.cityId);
      const block = MOCK_BLOCKS.find(b => b.blockId === request.blockId && b.branchId === request.branchId);
      ...
      if (!training) {
        ...
        result.failedRecords = request.selectedIds.map(selectedId => ({ selectedId, reason: 'Training program not found.' }));
        return of(result).pipe(delay(800));
      }

      if (!training.active) {
        result.failedCount = request.selectedIds.length;
        result.failedRecords = request.selectedIds.map(selectedId => ({ selectedId, reason: 'Selected training program is currently inactive.' }));
        return of(result).pipe(delay(800));
      }

      if (!city || !block) {
        result.failedCount = request.selectedIds.length;
        result.failedRecords = request.selectedIds.map(selectedId => ({ selectedId, reason: 'Invalid city, branch, or block selection.' }));
        return of(result).pipe(delay(800));
      }
      ...
      if (city.availableBudget === undefined || city.availableBudget < requiredBudget) {
        result.failedCount = request.selectedIds.length;
        result.failedRecords = request.selectedIds.map(selectedId => ({ selectedId, reason: 'City budget is insufficient for this batch.' }));
        return of(result).pipe(delay(800));
      }

      if (block.availableVacancy === undefined || block.availableVacancy < request.selectedIds.length) {
        result.failedCount = request.selectedIds.length;
        result.failedRecords = request.selectedIds.map(selectedId => ({ selectedId, reason: 'Block vacancy is insufficient for this batch.' }));
        return of(result).pipe(delay(800));
      }

      request.selectedIds.forEach(selectedId => {
        const candidate = MOCK_SELECTED.find(c => c.selectedId === selectedId);
        if (!candidate) {
          result.failedCount++;
          result.failedRecords?.push({ selectedId, reason: 'Selected candidate not found.' });
          return;
        }

        if (candidate.status === 'TRAINING_ASSIGNED' || candidate.status === 'MOVED_TO_TRAINEE') {
          result.skippedCount++;
          result.skippedRecords?.push({ selectedId, reason: 'Candidate is already assigned to training or has already progressed.' });
          return;
        }
