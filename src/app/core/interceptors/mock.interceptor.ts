...
import { InMemoryDataStore, EntityType, PagedResponse } from '../mocks/in-memory-data-store';
...
export class MockInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    return this.handleMockRequest(req).pipe(
      delay(this.randomDelay(100, 500))
    );
  }
   ...
   * Handle GET requests
   */
  private handleGet(urlParts: UrlParts, req: HttpRequest<any>): Observable<HttpEvent<any>> {
    ...
    if (id) {
      ...
      if (!entity) {
        return this.createErrorResponse(404, `${entityType} with id ${id} not found`);
      }
   ...
   */
  private handlePost(urlParts: UrlParts, body: any): Observable<HttpEvent<any>> {
  ...
  private handlePut(urlParts: UrlParts, body: any): Observable<HttpEvent<any>> {
    ...
    if (!entityType || !id) {
      return this.createErrorResponse(400, 'Invalid request: entity type and ID required');
    }
   ...
   */
  private createSuccessResponse(body: any, status: number): Observable<HttpEvent<any>> {
   ...
   */
  private createErrorResponse(status: number, message: string): Observable<HttpEvent<any>> {
